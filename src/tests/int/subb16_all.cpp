...
void validate_sub(uint16_t x, uint16_t y)
{
    for (int i = 0; i < 3; ++i)
    {
        {
            ...
            uint16sw_t c = uint16sw_t::sub_borrow(a, b, swcarry);

            uint8_t hwcarry = static_cast<uint8_t>(i);
            uint16_t z;
            hwcarry = _subborrow_u16(hwcarry, x, y, &z);
...

        {
            ...
            uint8_t swcarry = static_cast<uint8_t>(i);
            int16sw_t c = int16sw_t::sub_borrow(a, b, swcarry);
